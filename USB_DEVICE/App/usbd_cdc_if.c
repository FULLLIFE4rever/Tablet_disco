...
uint8_t RxBufferFSsize, RxBufferFSresive;
...
static int8_t
CDC_Receive_FS(uint8_t* pbuf, uint32_t* Len);
static int8_t
CDC_TransmitCplt_FS(uint8_t* pbuf, uint32_t* Len, uint8_t epnum);
...
CDC_Receive_FS(uint8_t* Buf, uint32_t* Len)
{
  ...
  if (*Len == 64 && UserRxBufferFS[1] == 0x0E &&
      UserRxBufferFS[(*Len) - 1] != 0xC7) {
    USBD_CDC_SetRxBuffer(&hUsbDeviceFS, &Buf[64]);
    RxBufferFSsize += *Len;
  } else {
    USBD_CDC_SetRxBuffer(&hUsbDeviceFS, UserRxBufferFS);
    RxBufferFSsize += *Len;
    Serial_Read(UserRxBufferFS, RxBufferFSsize);
    RxBufferFSsize = 0;
  }
 ...
 */
static int8_t
CDC_TransmitCplt_FS(uint8_t* Buf, uint32_t* Len, uint8_t epnum)
{
  uint8_t result = USBD_OK;
  /* USER CODE BEGIN 13 */
  UNUSED(Buf);
